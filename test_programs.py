...
program = [workout, "Rest", workout, "Rest", workout, "Rest", "Rest"]
...
url = "http://127.0.0.1:5001/users/" + username + "/programs"
headers = {"Content-Type": "application/json"}

success = requests.post(url, json=json.dumps(payload), headers=headers)

url = "http://127.0.0.1:5001/users/" + username + "/programs/1"
payload = {
	"program_name": "bro split",
    "username": username,
	"program": program
}
